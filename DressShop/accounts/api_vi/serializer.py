...
# Define UserLoginSerializer class
class UserLoginSerializer(TokenObtainPairSerializer):
    def validate(self,attr):
        users = User.objects.all()
        user = User.objects.get(phone=attr['phone'])
        verified_user = User.objects.filter(is_verified=user.is_verified)
        # print('%%%%%', verified_user)
        profile = Profile.objects.get(user_id=user.id)
        for user in users:
            if profile:
                # print('profile has been created before')
                pass
